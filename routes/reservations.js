...
router.get('/:reservationId', async (req, res, next) => {
  ...
  try {
    const [reservation] = await query('SELECT * FROM reservation WHERE id=?', [reservationId])
...
async function getReservableRooms(query, username) {
  const belongs_orgs = await query('SELECT * FROM belongs_organizations WHERE username=?', [
    username
  ])
  ...
  for (const belongs_org of belongs_orgs) {
    ...
    for (const roomByOrg of roomsByOrg) {
      if (!reservableRooms.find(room => room.id === roomByOrg.room_id)) {
        const [room] = await query('SELECT * FROM rooms WHERE id=?', [roomByOrg.room_id])
