...
router.get('/', async (req, res, next) => {
  ...
  try {
    ...
    for (const belongsOrg of belongsOrgs) {
      const roomsByOrg = await query('SELECT * FROM reservable_rooms WHERE organization_id=?', [
    ...
    for (const reservableRoom of reservableRooms) {
      const [room] = await query('SELECT * FROM rooms WHERE id=?', [reservableRoom])
      ...
      for (const reservation of reservations) {
        const [user] = await query('SELECT * FROM users WHERE username=?', [reservation.username])
        const reservationStart = new Date(`2001/01/01 ${reservation.start_time}`)
