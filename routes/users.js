...
const storage = multer.diskStorage({
  ...
  filename(req, file, cb) {
    ...
    cb(null, `${req.user.username}-${Date.now()}.${uuid()}.${mimes.extension(file.mimetype)}`)
  }
})
...
router.post('/', uploads.single('icon'), async (req, res, next) => {
  ...
  try {
    ...
    await query('DELETE FROM belongs_organizations WHERE username=?', [username])
    for (let i = 0; i < organization.length; i++) {
      await query('INSERT INTO belongs_organizations (organization_id, username) VALUES (?, ?)', [
        organization[i],
        username
      ])
    }
    ...
    if (filename) {
      await query('UPDATE users SET first_name=?, last_name=?, icon=? WHERE username=?', [
        first_name,
        last_name,
        filename,
        username
      ...
      await query('UPDATE users SET first_name=?, last_name=? WHERE username=?', [
        first_name,
        last_name,
        username
      ])
...
async function renderUserPage(username, req, res, validateMessage) {
  ...
  const belongsOrgs = await query('SELECT * FROM belongs_organizations WHERE username=?', [
    username
  ])
  const orgs = []
  for (const belongsOrg of belongsOrgs) {
    const orgId = belongsOrg.organization_id
    const [org] = await query('SELECT * FROM organizations WHERE id=?', [orgId])
    orgs.push(org)
  }
